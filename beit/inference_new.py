...
def main(args, ds_init):
    ...
    ROOT = "/datahdd/nhanv/git/VAIPE_AI/"
    obj_detect_output = pd.read_csv(ROOT + "yolov7/results.csv")
    print(obj_detect_output)
    pill2pres = {}
    with open(ROOT + "datasets2/RELEASE_private_test/pill_pres_map.json","r",encoding="utf-8") as fr:
        data = json.load(fr)
    for e in data:
        for pill in data[e]:
            pill2pres[pill.split(".")[0]] = e.split(".")[0].replace('VAIPE_P_TEST_NEW', 'VAIPE_P_TEST')
    ...
    for idx, row in tqdm(obj_detect_output.iterrows()):
        image_path = ROOT + "datasets2/RELEASE_private_test/pill/image/"+row.image_name
        ...
        fw.write(",".join([image_path.split("/")[-1],str(preds_lb[0]),str(probs[0]),str(row.x_min),str(row.y_min),str(row.x_max),str(row.y_max)]))
        fw.write("\n")
    fw.close()
