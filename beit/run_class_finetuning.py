...
def main(args, ds_init):
    ...
    if loss_type == 'BalancedCE' or loss_type =='WeightedCE':
        ...
        for data_iter_step, (samples, targets, preses) in tqdm(enumerate(data_loader_train)):
            labels_list = targets.cpu().detach().numpy()
            for i in range(n_classes):
                cls_num_list[i] = cls_num_list[i] + (labels_list == i).sum()
    ...
    criterion = get_loss_function(loss_type,args.nb_classes,**loss_params)
    print("Using loss {}".format(criterion))
