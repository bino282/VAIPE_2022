...
def create_pres_matrix():
    ...
    for i in range(matrix.shape[0]):
        for j in range(matrix.shape[1]):
            if i==j:
                matrix[i][j]=1
    with open(ROOT + "resources/train_pres2pill_label.json","r",encoding="utf-8") as fr:
        data = json.load(fr)
        for k in data:
            value = data[k]
...
with open(ROOT + "resources/pill_id2name.json","r",encoding="utf-8") as fr:
    data = json.load(fr)
...
count_ = 0
train_pres2pid = {}
with open(ROOT + "resources/diagnose_pill_id2name.json","r",encoding="utf-8") as fr:
...
# print(diagnose[key_])
# exit()
with open(ROOT + "resources/text_classification/corpus_test.txt","r",encoding="utf-8") as lines:
