...
class RecallCrossEntropy(torch.nn.Module):
    def __init__(self, n_classes=19, ignore_index=-100,device=None):
        super(RecallCrossEntropy, self).__init__()
        self.n_classes = n_classes
        self.ignore_index = ignore_index

    def forward(self, input, target,preses=None): 
        ...
        # gt_count[gt_idx==self.ignore_index] = gt_count_tmp[1]
        ...
        # # map ignored label to an exisiting one
        # fn_count[fn_idx==self.ignore_index] = fn_count_tmp[1]
        ...
        CE = F.cross_entropy(input, target, reduction='mean')
